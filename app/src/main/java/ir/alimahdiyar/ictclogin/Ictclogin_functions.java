...
public class Ictclogin_functions {
    ...
    public static String doLogin(String username, String password, String username2, String password2){
        if(username.isEmpty())
        ...
        try {
            ...
            if(error.equals("")) {
                ...
                if(username2.isEmpty())
                    ...
                    try {
                        //Toast.makeText(context, "Getting ictc...", Toast.LENGTH_SHORT).show();
                        doc = Jsoup.connect("http://ictc.hs").get();
                        chapID = doc.select("input[name=chap-id]").get(0).val();
                        chapChallenge = doc.select("input[name=chap-challenge]").get(0).val();

                        data1 = Jsoup.connect("http://ictc.hs/login").data("username", username2).data("password", hexMD5(chapID + password2 + chapChallenge)).data("chap-challenge", chapChallenge).data("chap-id", chapID).data("system-code", doc.select("input[name=system-code]").get(0).val()).data("dst", doc.select("input[name=dst]").get(0).val()).data("popup", doc.select("input[name=popup]").get(0).val()).data("error-code", doc.select("input[name=error_code]").get(0).val());

                        temp = doc.select("input[name=system-server]");
                        if(temp.size() > 0) {
                            data1.data("system-server", temp.get(0).val());
                        }

                        doc = data1.get();
                        String error2 = doc.select("input[name=error_code]").get(0).val();
                        if(error2.equals("")) {
                            //Toast.makeText(context, "Login Request Sent (Account2)", Toast.LENGTH_SHORT).show();
                            return getErrorText(error) + " (Account1)\n" + "Login Request Sent (Account2)";
                        }
                        else {
                            //Toast.makeText(context, getErrorText(error2) + " (Account2)", Toast.LENGTH_SHORT).show();
                            return getErrorText(error) + " (Account1)\n" + getErrorText(error2) + " (Account2)";
                        }
                    }
                    catch (Exception e) {
                        //Toast.makeText(context, e.getMessage() + " (Account2)", Toast.LENGTH_SHORT).show();
                        return getErrorText(error) + " (Account1)\n" + e.getMessage() + " (Account2)";
                    }
            }
        }
        catch (Exception e) {
            if(e.getMessage().equals("Invalid index 0, size is 0")) {
    ...
    private static String hexMD5(String input){
        ...
        result = duktape.evaluate(script).toString();
        return result;
    }
